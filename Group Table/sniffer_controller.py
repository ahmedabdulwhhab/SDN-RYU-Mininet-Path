...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if datapath.id == 1:
            # add group tables
            self.send_group_mod(datapath,datapath.id)
            actions = [parser.OFPActionGroup(group_id=10)]
            # forward in 2 to ports 1,3,4
            match = parser.OFPMatch(in_port=2)
            self.add_flow(datapath, 10, match, actions)
            # forward in 3 to ports 1,2,4
            actions = [parser.OFPActionGroup(group_id=20)]
            match = parser.OFPMatch(in_port=3)
            self.add_flow(datapath, 10, match, actions)
            # forward in 4 to ports 1,3,2
            self.send_group_mod(datapath,datapath.id)
            actions = [parser.OFPActionGroup(group_id=30)]
        ...
        if datapath.id == 2:
            ...
            self.send_group_mod(datapath,datapath.id)
            actions = [parser.OFPActionGroup(group_id=10)]
            # forward in 3 to ports 1,2
            match = parser.OFPMatch(in_port=3)
            self.add_flow(datapath, 10, match, actions)
            # forward in 2 to ports 1,3
            actions = [parser.OFPActionGroup(group_id=20)]
            match = parser.OFPMatch(in_port=2)
            self.add_flow(datapath, 10, match, actions)
    ...
    def send_group_mod(self, datapath,dp_id):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
